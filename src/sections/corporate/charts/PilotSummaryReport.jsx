...
const ReportPart5 = ({ dateRange }) => {
    ...
    const cachedData = useAppSelector((state) =>
        dateRange.startDate && dateRange.endDate
            ? selectReportData(state, 'approvalCountReport', dateRange.startDate, dateRange.endDate)
            : null
    );
    const loading = useAppSelector((state) =>
        dateRange.startDate && dateRange.endDate
            ? selectReportLoading(state, 'approvalCountReport', dateRange.startDate, dateRange.endDate)
            : false
    );
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (cachedData) {
                setTeamLeadData(Array.isArray(cachedData[0]) ? cachedData[0] : []);
                return;
            }

            // Fetch from API via Redux
            try {
                const result = await dispatch(fetchApprovalCountReport({
                    startDate: dateRange.startDate,
                    endDate: dateRange.endDate
                }));

                if (fetchApprovalCountReport.fulfilled.match(result)) {
                    setTeamLeadData(Array.isArray(result.payload.data[0]) ? result.payload.data[0] : []);
                } else {
                    setTeamLeadData([]);
                }
            } catch (error) {
