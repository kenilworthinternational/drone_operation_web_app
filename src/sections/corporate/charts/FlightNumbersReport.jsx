...
const ReportPart6 = ({ dateRange }) => {
    ...
    const cachedData = useAppSelector((state) =>
        dateRange.startDate && dateRange.endDate
            ? selectReportData(state, 'flightNumbersReport', dateRange.startDate, dateRange.endDate)
            : null
    );
    const loading = useAppSelector((state) =>
        dateRange.startDate && dateRange.endDate
            ? selectReportLoading(state, 'flightNumbersReport', dateRange.startDate, dateRange.endDate)
            : false
    );
    const selectedRegions = useAppSelector((state) => selectReportFilter(state, 'selectedRegions'));
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const result = await dispatch(fetchFlightNumbersReport({
                    startDate: dateRange.startDate,
                    endDate: dateRange.endDate
                }));

                if (fetchFlightNumbersReport.fulfilled.match(result)) {
        ...
        fetchData();
    }, [dateRange, selectedRegions, cachedData, dispatch]);
            ...
            <div className="mb-4 flex justify-center text-center items-center">
                ...
                {regions.length > 0 ? (
                    <select
                        ...
                        onChange={(e) => dispatch(setReportFilter({ filterName: 'selectedRegions', value: e.target.value }))}
