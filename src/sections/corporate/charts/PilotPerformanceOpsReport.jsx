...
import { fetchPilotPerformance, clearPilotsData } from '../../../components/pilotPerformanceSlice';
import * as XLSX from 'xlsx';

const ReportPart3 = ({ dateRange }) => {
  const dispatch = useDispatch();
  const { pilotsData, loading, error } = useSelector(state => state.pilotPerformance);

  useEffect(() => {
    // Check if dateRange is empty or invalid
    if (!dateRange.startDate || !dateRange.endDate) {
      dispatch(clearPilotsData());
      return;
    }

    // Dispatch the async thunk to fetch data
    dispatch(fetchPilotPerformance({
