...
const BrokerManagement = () => {
  ...
  const handleUpdateBroker = async (e) => {
    ...
    try {
      const result = await dispatch(updateBrokerThunk(editFormData));
      
      if (updateBrokerThunk.fulfilled.match(result) && result.payload.status === 'true') {
  ...
  const handleToggleStatus = async (broker) => {
    ...
    try {
      const result = await dispatch(updateBrokerStatusThunk({ brokerId: broker.id, status: newStatus }));
      
      if (updateBrokerStatusThunk.fulfilled.match(result) && result.payload.status === 'true') {
            ...
            <div className="modal-body-bro-manage">
              <form onSubmit={handleUpdateBroker} className="edit-form-bro-manage">
                ...
                <div className="form-actions-bro-manage">
                  ...
                  >
                    {loading.updating ? 'Updating...' : 'Update Broker'}
                  </button>
                </div>
              </form>
            </div>
