...
const IncompleteOpsRoomRejected = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await dispatch(baseApi.endpoints.getIncompleteSubtasks.initiate({
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `Incomplete_Ops_Room_Rejected_${pilotPart}${formatDateForFilename(startDate)}_to_${formatDateForFilename(endDate)}.xlsx`);
  };
  ...
  const downloadPDF = () => {
    ...
    doc.save(`Incomplete_Ops_Room_Rejected_${pilotPart}${formatDateForFilename(startDate)}_to_${formatDateForFilename(endDate)}.pdf`);
  };
...
export default IncompleteOpsRoomRejected;