...
import { FiCheckCircle, FiUser, FiUserX, FiMapPin, FiPhone, FiDownload } from "react-icons/fi";
...
const OpsAssign = () => {
  ...
  const operatorsLoading = useAppSelector((state) => state.operators.loading.operators);
  const assignLoading = useAppSelector((state) => state.operators.loading.assignOperator);
  const planOperatorsLoading = useAppSelector((state) => state.operators.loading.planOperators);
  const operatorsError = useAppSelector((state) => state.operators.error);
  ...
  // Trigger default search for assigned plans after operators load
  useEffect(() => {
  ...
  // Operators are now loaded via Redux
  ...
  const handleDateRangeChange = async () => {
    ...
    try {
      const result = await dispatch(fetchPlanOperatorsByDateRange({ startDate, endDate }));
