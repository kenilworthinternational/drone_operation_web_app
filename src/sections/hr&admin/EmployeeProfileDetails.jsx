...
import { useGetAllEmployeeRegistrationsQuery, useForwardToPayrollMutation } from '../../api/services NodeJs/jdManagementApi';
import {
  PersonalTab,
  EmploymentTab,
  FamilyTab,
...
import EmployeeProfileHero from './employeeProfile/EmployeeProfileHero';
import {
  TAB_GROUPS,
  DEFAULT_GROUP,
  DEFAULT_TAB,
  findGroupForTab,
} from './employeeProfile/tabConfig';
import ProfileRecordWithFileSection from './employeeProfile/ProfileRecordWithFileSection';
import MasterSelect from './employeeProfile/MasterSelect';
import {
...
const SECTION_DEFS = {
  ...
  'employment-history': {
    ...
    fields: [
      { name: 'previous_employment', label: 'Previous Employment' },
      { name: 'company_name', label: 'Company Name' },
      { name: 'industry', label: 'Industry' },
      { name: 'designation', label: 'Designation' },
      { name: 'employment_period', label: 'Employment Period' },
...
function GenericListSection({ section, def, employeeId }) {
...
function SingleSection({ section, def, employeeId }) {
  const { data, isLoading, refetch } = useListEmployeeProfileSectionQuery({ section, employeeId });
  const [saveSection, { isLoading: saving }] = useSaveEmployeeProfileSectionMutation();
    ...
    <div className="epd-section">
      ...
      <div className="epd-table-wrap">
        {isLoading ? (
          ...
          <table className="epd-table">
            <thead>
              <tr><th>Type</th><th>File</th><th>Expiry</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {docs.map((doc) => (
...
const EmployeeProfileDetails = () => {
  ...
  const renderTabContent = () => {
    ...
    if (activeTab === 'personal') {
      return <PersonalTab employeeId={employeeId} key={`personal-${employeeId}`} />;
    }
    if (activeTab === 'employment') {
      return <EmploymentTab employeeId={employeeId} key={`employment-${employeeId}`} />;
    }
    if (activeTab === 'family') {
      return <FamilyTab employeeId={employeeId} key={`family-${employeeId}`} />;
    }
    if (activeTab === 'security') {
      return <SecurityTab employeeId={employeeId} key={`security-${employeeId}`} />;
    }
    if (activeTab === 'education') {
      return (
        <ProfileRecordWithFileSection
          ...
          intro="Add each qualification with its certificate or transcript. One document per education record."
        />
      );
    }
    if (activeTab === 'employment-history') {
      return (
        <ProfileRecordWithFileSection
          ...
          intro="Add each previous employer with its service letter. One letter per employment record."
        />
      );
    }
    if (activeTab === 'documents') {
      return (
        <div key={`documents-${employeeId}`}>
          <LegacyFilesSection employeeId={employeeId} />
    ...
    if (SECTION_DEFS[activeTab]?.single) {
      return (
        <SingleSection section={activeTab} def={SECTION_DEFS[activeTab]} employeeId={employeeId} key={`${activeTab}-${employeeId}`} />
      );
    }
    if (SECTION_DEFS[activeTab]) {
      return (
        <GenericListSection section={activeTab} def={SECTION_DEFS[activeTab]} employeeId={employeeId} key={`${activeTab}-${employeeId}`} />
      );
    }
    return null;
  };
      ...
      <div className="ep-layout">
        ...
        <main className="ep-main">
          {!employeeId ? (
            ...
            <div className="ep-main-profile">
              <EmployeeProfileHero employeeId={employeeId} key={`hero-${employeeId}`} />
