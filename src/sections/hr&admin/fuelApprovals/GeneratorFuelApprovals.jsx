...
} from '../../../api/services NodeJs/generatorFuelApprovalsApi';
...
const GeneratorFuelApprovals = ({ embedded = false }) => {
  ...
  const [decide, { isLoading: deciding }] = useDecideGeneratorFuelApprovalMutation();
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1.5 }}>
            <Box>
              <Typography variant="h6" sx={{ fontWeight: 700, color: '#102739' }}>
                Generator Fuel Approvals
              </Typography>
              <Typography variant="caption" sx={{ color: '#5f7383' }}>
                Approve or decline pilot-submitted generator fuel bills. Approval debits the linked card balance.
              </Typography>
            </Box>
            ...
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow sx={{ backgroundColor: '#f4f8fc' }}>
                  ...
                  <TableCell sx={{ fontWeight: 700 }}>Pilot</TableCell>
                ...
                {rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} align="center" sx={{ py: 4, color: '#7c8a99' }}>
                      {tab === 0 ? 'No pending generator fuel approvals' : 'No history yet'}
                    </TableCell>
                  </TableRow>
                ) : (
                  rows.map((row) => (
                    <TableRow key={row.id} hover>
                      ...
                      <TableCell>
                        <div style={{ fontWeight: 600 }}>{row.pilot_name || '-'}</div>
                        <div style={{ fontSize: 11, color: '#7c8a99' }}>{row.pilot_mobile || ''}</div>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={!!decline} onClose={() => !deciding && setDecline(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 1.5, color: '#5f7383' }}>
            Please provide a reason. The pilot will see this on their app.
          </Typography>
          <TextField
            autoFocus
            multiline
            ...
            <Box sx={{ pt: 0.5 }}>
              ...
              <Box sx={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 1, p: 1.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="caption" sx={{ color: '#64748b' }}>Pilot</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 600 }}>
                    {approveConfirm.pilot_name || '-'}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="caption" sx={{ color: '#64748b' }}>Card</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 600 }}>
                    {approveConfirm.card_holder || approveConfirm.card_number || '-'}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
...
export default GeneratorFuelApprovals;
