...
import { pilotRevenue, defaultValues, addPilotRevenue, pilotsEarnedByRevenue, pilotCanceledReasons } from '../../api/api';
...
const Earnings = () => {
  ...
  const fetchDefaultValues = async (date) => {
    try {
      const formattedDate = date.toLocaleDateString('en-CA'); // YYYY-MM-DD format
      const response = await defaultValues(formattedDate);
      
      if (response && response.plantation_maximum && response.amount_if_stoped && response.minimum_ha_per_day && response.amount_per_ha_day) {
        setDefaultVals({
          plantation_maximum: response.plantation_maximum,
          amount_if_stoped: response.amount_if_stoped,
          minimum_ha_per_day: response.minimum_ha_per_day,
          amount_per_ha_day: response.amount_per_ha_day
        });
      }
    } catch (err) {
  ...
  const fetchDowntimeReasons = async () => {
    try {
      const response = await pilotCanceledReasons();
      if (response && Array.isArray(response)) {
        setDowntimeReasons(response);
      }
    } catch (err) {
      console.error('Error fetching downtime reasons:', err);
    }
  };

  // Fetch pilot revenue data
  const fetchPilotRevenue = async (date) => {
    if (!date) return;
    ...
    try {
      const formattedDate = date.toLocaleDateString('en-CA'); // YYYY-MM-DD format
      const response = await pilotRevenue(formattedDate);
      
      if (response && response.status === "true" && response.pilots) {
        setPilotData(response.pilots);
      } else {
        setPilotData([]);
        setError('No data found for the selected date');
      }
    } catch (err) {
      console.error('Error fetching pilot revenue:', err);
  ...
  const fetchSavedPilotData = async (date) => {
    ...
    try {
      const formattedDate = date.toLocaleDateString('en-CA'); // YYYY-MM-DD format
      const response = await pilotsEarnedByRevenue(formattedDate);
  ...
  const handleSavePilotData = async (pilot, stats) => {
    ...
    try {
      ...
      const response = await addPilotRevenue(
        pilot.pilot_id,
        formattedDate,
        parseFloat(stats.assigned),
        parseFloat(stats.opsRoomCovered),
        parseFloat(stats.canceled),
        parseFloat(stats.coveredRevenue),
        selectedDowntimeReasons[pilot.pilot_id] || '', // downtime_reason
        downtimeApprovals[pilot.pilot_id] || 0, // Send actual approval status (0=Pending, 1=Approved, 2=Declined)
        parseFloat(stats.downtimePayment),
        parseFloat(stats.dailyEarning),
        1 // verified
      );
  ...
  // Initial data fetch
  useEffect(() => {
    fetchDefaultValues(selectedDate);
    fetchPilotRevenue(selectedDate);
