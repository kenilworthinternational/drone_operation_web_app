...
import {
  dateRangAscBookings,
  updateDatePlannedAscBooking,
  updateMissionPlannedAsc,
  getSectors,
  cropType,
  missionType,
  chemicalTypeList,
...
const BookingList = () => {
  // removed unused expandedIndex state
  ...
  // removed unused editingMissionId state
  const [updating, setUpdating] = useState(false);
  // removed unused pendingPaymentType state
  ...
  // removed unused brokerId state
  const [showFilters, setShowFilters] = useState(false);
  const [selectedStatusFilter, setSelectedStatusFilter] = useState('all');
  const [showPickedOnly, setShowPickedOnly] = useState(false);
  const [defaultQueueView, setDefaultQueueView] = useState(true); // New state for default queue view
  // ASC + GND assignment popup state
  ...
  const [modalMode, setModalMode] = useState('details'); // 'details' or 'edit'
  const [selectedBooking, setSelectedBooking] = useState(null);

  // Update the dropdown data fetching effect
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        const [sectorsRes, cropsRes, missionsRes, timeRes, ascRes, brokersRes] = await Promise.all([
          getSectors(),
          cropType(),
          missionType(),
          getTimePick(),
        ...
        setSectorOptions(sectorsRes?.data || sectorsRes || []);
        setCropOptions(cropsRes?.data || cropsRes || []);
        setMissionOptions(missionsRes?.data || missionsRes || []);
        setTimeOptions(timeRes?.data || timeRes || []);
        setAscOptions(processedAsc);
        setBrokerOptions(brokersRes?.brokers || []);
      } catch (error) {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError('');
      const response = await dateRangAscBookings(
  ...
  }, [startDate, endDate, ascOptions, timeOptions]);
  ...
  useEffect(() => {
    const fetchChemicals = async () => {
      if (editableData.mission_type) {
        try {
          const chemicalsRes = await chemicalTypeList();
  ...
  const handleUpdate = async (missionId) => {
    try {
      ...
      const response = await updateMissionPlannedAsc(dataSet);
      console.log(dataSet);
  ...
  const handleConfirmPlannedDate = async (missionId) => {
    ...
    try {
      setUpdating(true);
      const response = await updateDatePlannedAscBooking(missionId, dateToSend, paymentType);
  ...
  const handleConfirmAscGnd = async () => {
    try {
      ...
      const response = await setAscForMission(payload);
      if (response?.status === "true") {
  ...
  const handleStatusChange = async (missionId, newStatus) => {
    try {
      ...
      const response = await setStatusAscPlan(missionId, newStatus);
