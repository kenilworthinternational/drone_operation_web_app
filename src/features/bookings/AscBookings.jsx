...
import {
    farmerDetailsAscBooking,
    cropType,
    missionType,
    chemicalTypeList,
    getStages,
    submitDataAscBooking,
    addFarmer,
    updateFarmer,
    getTimePick,
    getSectors,
...
const AscBookings = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await Promise.allSettled(dataFetchConfig.map(async (config) => {
                const { key, apiFn, setData, setLoading, setError, dependsOn, processResponse } = config;
    ...
    const handleAddFarmer = async () => {
        ...
        try {
            ...
            const farmerResponse = await addFarmer({
                name: farmerName,
                nic: nic,
                address: address,
                telephone: mobile
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const missionPromises = lands.map(async (land) => {
                ...
                return await submitDataAscBooking(bookingData);
            });
    ...
    const handleSearch = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await farmerDetailsAscBooking(searchId);
