...
import { canceledFieldsByDateRange } from '../../api/api';
...
const OpsReport7 = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await canceledFieldsByDateRange(
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `Canceled_Fields_Report_${estatePart}_${reasonPart}_${pilotPart}_${formatDate(startDate)}_to_${formatDate(endDate)}.xlsx`);
  };
  ...
  const downloadPDF = () => {
    ...
    const estatePart = estateFilter ? estateFilter.replace(/\s+/g, '_') : 'All_Estates';
    const reasonPart = reasonFilter ? reasonFilter.replace(/\s+/g, '_') : 'All_Reasons';
    doc.save(`Canceled_Fields_Report_${estatePart}_${reasonPart}_${pilotPart}_${formatDate(startDate)}_to_${formatDate(endDate)}.pdf`);
  };
