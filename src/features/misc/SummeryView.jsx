...
const SummeryView = () => {
  ...
  const handleSearch = async () => {
    ...
    // Fetch plans for the start date (since this is the main date for the search)
    await fetchPlansForDate(startDate);
  };
  ...
  const formatData = (data) => {
    ...
    return data.map(item => {
      // Separate teams into leaders and pilots
      const formattedTeams = item.selectedPilots?.map((pilot, index) => {
        ...
        return {
          ...
          drone: drone || null  // Attach the drone data
        };
      }) || [];
  
      // Separate out the selected drones and pilots
      const selectedDrones = item.selectedDrones || [];
      const selectedPilots = formattedTeams.filter(pilot => !pilot.isLeader);  // Only pilots, no leaders
      const teamLeaders = formattedTeams.filter(pilot => pilot.isLeader);  // Only leaders
      ...
      return {
        ...
        selectedDrones,  // New data for drones
        selectedPilots,  // New data for pilots
        teamLeaders      // New data for team leaders
      };
    });
  };
          ...
          {state.summaryData.length > 0 ? (
            formatData(state.summaryData).map((data, index) => (
              <SummaryWidget
                ...
                teams={data.teams}
                selectedDrones={data.selectedDrones}  // Pass drones separately
                selectedPilots={data.selectedPilots}  // Pass pilots separately
                teamLeaders={data.teamLeaders}        // Pass team leaders separately
              />
            ))
          ) : (
