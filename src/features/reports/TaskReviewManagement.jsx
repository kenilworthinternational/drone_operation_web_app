...
import { 
  viewTaskReportByDateRange, 
  updateReviewForFlagByReviewBoard, 
  updateReviewForFlagByDirectorOps 
} from '../../api/api';
...
const ReportReview = () => {
  ...
  const [filterManagerReview, setFilterManagerReview] = useState('all'); // all, pending, reviewed
  const [filterDirectorOps, setFilterDirectorOps] = useState('all'); // all, pending, approved, rejected
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const response = await viewTaskReportByDateRange(fromDateStr, toDateStr);
      
      if (response && response.flags) {
        let filteredReports = response.flags;
  ...
  useEffect(() => {
    fetchReports();
  }, [fromDate, toDate, filterManagerReview, filterDirectorOps]);
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      if (reviewType === 'manager') {
        response = await updateReviewForFlagByReviewBoard(
          selectedReport.task_id,
          reviewText
        );
      } else if (reviewType === 'director') {
        response = await updateReviewForFlagByDirectorOps(
          selectedReport.task_id,
          ...
          <div className="status-filters">
            <div className="filter-group">
              ...
              <select 
                ...
                onChange={e => setFilterManagerReview(e.target.value)}
            ...
            <div className="filter-group">
              ...
              <select 
                ...
                onChange={e => setFilterDirectorOps(e.target.value)}
                className="filter-select"
              >
                <option value="all">All</option>
                <option value="pending">Pending</option>
