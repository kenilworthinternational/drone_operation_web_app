...
const IndividualPilotSummary = () => {
  ...
  const fetchPilotRevenueData = async (start, end) => {
    ...
    setError('');
    try {
      const response = await pilotsEarnedByRevenueDateRange(start, end);
      ...
      console.error('Error fetching pilot revenue data:', error);
      setError('Error loading pilot revenue data');
      setReportData([]);
    } finally {
      setLoading(false);
    }
